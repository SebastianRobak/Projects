...
bool WriteToFile(string cub_filename, string pr1_filename, string pr2_filename, string pr3_filename, string pr4_filename, Cuboid cub, Prism prism[])
{
  ofstream  file_stream;

  file_stream.open(cub_filename);
  if (!file_stream.is_open())  {
    cerr << ":(  Operacja otwarcia do zapisu \"" << cub_filename << "\"" << endl
	 << ":(  nie powiodla sie." << endl;
    return false;
  }

  Cub_WriteToStream(file_stream,cub);

  file_stream.close();
  //return !file_stream.fail();


  file_stream.open(pr1_filename);
  if (!file_stream.is_open())  {
    cerr << ":(  Operacja otwarcia do zapisu \"" << pr1_filename << "\"" << endl
	 << ":(  nie powiodla sie." << endl;
    return false;
  }

  Prism_WriteToStream(file_stream,prism[0], cub[4]);

  file_stream.close();


  file_stream.open(pr2_filename);
  if (!file_stream.is_open())  {
    cerr << ":(  Operacja otwarcia do zapisu \"" << pr2_filename << "\"" << endl
	 << ":(  nie powiodla sie." << endl;
    return false;
  }

  Prism_WriteToStream(file_stream,prism[1], cub[5]);

  file_stream.close();


  file_stream.open(pr3_filename);
  if (!file_stream.is_open())  {
    cerr << ":(  Operacja otwarcia do zapisu \"" << pr3_filename << "\"" << endl
	 << ":(  nie powiodla sie." << endl;
    return false;
  }

  Prism_WriteToStream(file_stream,prism[2], cub[6]);

  file_stream.close();


  file_stream.open(pr4_filename);
  if (!file_stream.is_open())  {
    cerr << ":(  Operacja otwarcia do zapisu \"" << pr4_filename << "\"" << endl
	 << ":(  nie powiodla sie." << endl;
    return false;
  }

  Prism_WriteToStream(file_stream,prism[3], cub[7]);
...
bool ObstacleWriteToFile(string filename, Drone *drone)
{
  ofstream  file_stream;
  ...
  Cub_WriteToStream(file_stream,drone->body());

for (int i=0; i<4; ++i)
{
  Prism_WriteToStream(file_stream,drone->rotor(i), drone->body()[i+4]);
}
...
bool ObstacleWriteToFile(string filename, Cuboid cub)
{
  ofstream  file_stream;
...
Obj = Fact.Nazwa("Robot A").StworzObiekt(TO_Robot, mid, dim);
